...
class DragoSystemApp(ctk.CTk):
    ...
    def show_empresas(self):
        ...
        for emp in self.empresas:
            ...
            # ── Info: perfil buscado y requisitos ─────────────────────────────
            carrera = random.choice(self.carreras)
            ctk.CTkLabel(frame, text=f"Buscando Perfil: {carrera}",
                        font=("Segoe UI", 12, "bold")).pack(anchor="w", padx=25, pady=(0, 2))
            ctk.CTkLabel(frame, text=f"Requisitos: {self.generar_habilidades(carrera)}",
                        text_color=self.colors["texto_secundario"],
                        wraplength=600, justify="left").pack(anchor="w", padx=25, pady=(0, 10))
